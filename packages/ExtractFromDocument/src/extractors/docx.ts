...
function openReadStream(zipFile: ZipFile, entry: Entry): Promise<string> {
	return new Promise<string>((resolve, reject) => {
		...
		zipFile.openReadStream(entry, (error, readStream) => {
			if (error) return reject(new Error(`Failed to read entry: ${error.message}`));
...
async function extract(buffer: Buffer, args: IDocumentOptions): Promise<IDocumentExtract> {
	...
	return new Promise<IDocumentExtract>((resolve, reject) => {
		// Open the DOCX file as a zip archive
		yauzl.fromBuffer(buffer, { lazyEntries: true }, (error, zipFile) => {
			if (error) return reject(new Error(`Failed to open DOCX file: ${error.message}`));
			...
			const processEntry = async (entry: yauzl.Entry) => {
				// Skip entries that are not the main document
				if (entry.fileName !== "word/document.xml") return zipFile.readEntry();
